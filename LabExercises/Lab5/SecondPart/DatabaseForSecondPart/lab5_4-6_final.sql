...
/*CREATE VIEW sportLocation(sport, place) /* davame imena na kolonite v rezultantnata tablica (trqbva da e ednakuv na broq tablici v JOIN-ovete*/
/*AS
SELECT sports.name, sportGroups.location
from sports JOIN sportGroups
ON sports.id = sportGroups.sport_id;   /* */

/* VIEW - Virtualna tablica, kogato pishem INSERT zaqvki, dobavqme stoinosti i za stoinostite v tablicite, koito sa NULL*/
/* VIEW shte ima na kontrolnoto */

/*SELECT * FROM sportLocation; /* sushtoto kato ot red 252 do 256*/

/*DROP VIEW sportLocation;  /* iztrivane na VIEW */
...
FROM taxesPayments
group by group_id;*/
/* prilagame agregatnata funkciq vurhu otdelni chasti ot nqkoq kolona, zavisi ot stoinostite vutre; mojem poveche koloni*/

/*SELECT group_id, sum(paymentAmount)
FROM taxesPayments
GROUP BY group_id;*/
/*having sum(paymentAmount) > 11000;  printira 18150 v tablicata*/
/*WHERE sum(paymentAmount) > 1100;    ne mojem tuka da izpolzvame WHERE*/


/* group by i having na kontrolnoto*/
